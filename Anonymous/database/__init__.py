...
try:
    hasnainkk_db_client = MongoClient(DB_URI)
except PyMongoError as f:
    LOGGER.error(f"Error in Mongodb: {f}")
    exiter(1)
hasnainkk_main_db = hasnainkk_db_client[DB_NAME]
...
class MongoDB:
    ...
    def __init__(self, collection) -> None:
        self.collection = hasnainkk_main_db[collection]
    ...
    @staticmethod
    def close():
        return hasnainkk_db_client.close()
